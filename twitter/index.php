...
//$_SESSION['token'] = $_REQUEST['oauth_token'];
...
$_SESSION['token'] = $_REQUEST['oauth_token'];
$_SESSION['token_secret'] = $_REQUEST['oauth_token'];
		...
		if($twClient->http_code == '200'){
			...
			if(empty($fbid)){
				...
				if($id){	
					...
					$DB->Insert("INSERT INTO `members` ( `id` , `username` , `password` , `email` , `session` , `ip` , `lastlogin` , `reg_type`,`visible` , active, `created`, packageid, hits, profile_complete, templateid, updated, moderator, activate_code, highlight, ip_long, ip_lat, ip_country, ip_code,member_rating,  msgStatus,  video_duration,  video_live, facebook_id )VALUES (NULL , '".$fbname['0']."', '".md5($fbpasscode)."', '".$fbemail."', '".$fbsession."', '".$ip."', '".DATE_TIME."', 'Twitter','yes', '".$fbstatus."', '".DATE_TIME."', '3','0','0','1','".DATE_TIME."', 'no', 'OK','off','".$reg_long."','".$reg_lat."','".$reg_country."','".$reg_code."', '0','".eMeetingInput($MSGSTATUS)."','0','no','".$id."')");
					...
					if(isset($_POST['do'])){ 
						...
						$Error_Report =  ChangeDo($_POST['do'], $_POST,$obj, $MOBILE = "no");
