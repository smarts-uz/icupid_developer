...
switch ( $action ){
	...
	case 'getmarketdetail':
		$mid = $_POST['mid'];
		$market_detail = getMarketDetail($mid);
		return $market_detail;
	break;

	case 'addmarketdetail':
		$market_detail = addMarketDetail($_POST);
		return $market_detail;
	break;

	case 'updatemarketdetail':
		$market_update = updateMarketDetail($_POST);
		echo $market_update;
	break;

	case 'getDashboardFigures':
		$dashboard_figures = getDashboardFigures($_POST);
		echo json_encode($dashboard_figures);
	break;

	case 'getAdminReportFigures':
		$dashboard_figures = getAdminReportFigures($_POST);
		echo json_encode($dashboard_figures);
	break;
	...
	case "calrsssearch":{
			...
			if ($user and $password)
				...
				print '<ul class="form"><div class="box_body">	<form method="post" action="management.php" name="form1">
...
case "fieldtypepage": {
	...
	if(ADMIN_DEMO == "yes"){ print "Disabled in Demo Mode"; return; }
		...
		switch($type){
			...
			case "1": {
				...
				$DB->Update("UPDATE field SET browsepage='".$value."' WHERE fid='".$id."' LIMIT 1");
			
			} break;
			case "2": {
			
				$DB->Update("UPDATE field SET required='".$value."' WHERE fid='".$id."' LIMIT 1");
			
			} break;
			case "3": {
			
				$DB->Update("UPDATE field SET matchpage='".$value."' WHERE fid='".$id."' LIMIT 1");
	...
	case "ChangeDiv":{
		...
		switch($switchMe){
			...
			case "membership": {
				print "<select onChange=\"eMeetingSaveDiv('".$div."','membership',this.value,'".$id."')\">".DisplayPackage($current)."</select>";
			} break;
			case "status": {
				print "<select onChange=\"eMeetingSaveDiv('".$div."','status',this.value,'".$id."')\">".DisplayStatus($current)."</select>";
			} break;
			case "gender": {
				print "<select onChange=\"eMeetingSaveDiv('".$div."','gender',this.value,'".$id."')\">".DisplayGenderList($current)."</select>";
			} break;
			case "country": {
				print "<select onChange=\"eMeetingSaveDiv('".$div."','country',this.value,'".$id."')\">".DisplayCountryList($current)."</select>";
			} break;
