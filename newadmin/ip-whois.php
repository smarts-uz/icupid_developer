...
if (isset($_GET['ip'])) { $domain = $_GET['ip']; }
...
*/ ?>
<a href="#" onClick="window.close()">Close Window</a> - <a href="#" onClick="this.className='removelink';eMeetingIpWhoisBlock('<?php echo $_GET['uid'];?>');">Block Member</a><br />
</div>
...
$ip_shitlist = file("ip-shitlist.txt");

for ($i = 0; $i < count($ip_shitlist); $i++) {
$ip_owner = trim($ip_shitlist[$i]);
if (preg_match("/$ip_owner/i",$result))
   {
   $warning_txt .= "<font style=\"color:#cc0000; font-size:18px; background: #ffff99;\"><b>SCAMMER WARNING: IP Belongs To $ip_owner</b><br /></font><font style=\"color:#cc0000; font-size:14px;\"><i>* This IP should not be surfing web -- it is a server, not an ISP.<br />* This data center has been identified as a repeated source of scammers here.</i></font><br />&nbsp;<br />";
   }
} // end for $i
