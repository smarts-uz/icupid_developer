...
$sSql = "SELECT DISTINCT members.id, members.username, files.bigimage, members_data.location, members_data.age, members_network.type FROM members LEFT JOIN files ON ( files.uid = members.id AND files.default=1) LEFT JOIN members_data ON ( members_data.uid = members.id ) LEFT JOIN members_network ON ( members_network.uid = members.id OR members_network.to_uid = members.id ) WHERE (members_network.uid='".$iId."' OR  members_network.to_uid='".$iId."') AND members.id !='".$iId."' AND members.id != '0' AND members_network.type ='".$type."' LIMIT 0, 100";
