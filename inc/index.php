...
/**
	...
	if ($_SESSION['auth'] =="yes" && $page != "groups") {
		...
		/*if($data['active'] == 'suspended' || $data['active'] == 'unapproved' || $data['active'] == 'cancel'){	
			...
			header("location: ".DB_DOMAIN."index.php?dll=logout"); exit();
...
///////////////////////////////////////////////////////////////////////////////////////////
	...
	if(($Myprofile_count['profile_complete']=='0') || ($Myprofile_count['profile_complete'] < '40')){
		header("Location: ".DB_DOMAIN."index.php?dll=account&sub=edit");
	}
...
///////////////////////////////////////////////////////////////////////////////////////////
	...
	if(isset($PACKAGEACCESS[$_SESSION['packageid']]) && D_FREE =="no"){
		...
		if(in_array($PackageString,$PACKAGEACCESS[$_SESSION['packageid']])){ 
			...
			header("location: ".DB_DOMAIN."index.php?dll=subscribe");
...
switch($page){
	...
	case "index": {
		...
		if(file_exists('inc/templates/'.D_TEMP.'/config.php')){   
			...
			//echo $template_config['thomeLayout'];
   ...
   if(empty($code)) {
     ...
     $dialog_url = "http://www.facebook.com/dialog/oauth?client_id=" 
       ...
       . FACEBOOK_APP_ID . "&redirect_uri=" . urlencode($my_url) . "&scope=publish_actions,email";
...
//$DB->Insert("INSERT INTO `members` ( `id` , `username` , `password` , `email` , `session` , `ip` , `lastlogin` , `visible` , active, `created`, packageid, hits, profile_complete, templateid, updated, moderator, activate_code, highlight, ip_long, ip_lat, ip_country, ip_code,member_rating,  msgStatus,  video_duration,  video_live, facebook_id )VALUES (NULL , '".$name."', '".$fbpasscode."', '".$fbemail."', '".$fbsession."', '".$ip."', '".DATE_TIME."', 'yes', '".$fbstatus."', '".DATE_TIME."', '3','0','0','1','".DATE_TIME."', 'no', 'OK','off','".$reg_long."','".$reg_lat."','".$reg_country."','".$reg_code."', '0','".eMeetingInput($MSGSTATUS)."','0','no','".$id."')");
$DB->Insert("INSERT INTO `members` ( `id` , `username` , `password` , `email` , `session` , `ip` , `lastlogin` , `visible` , active, `created`, packageid, hits, profile_complete, templateid, updated, moderator, activate_code, highlight, ip_long, ip_lat, ip_country, ip_code,member_rating,  msgStatus,  video_duration,  video_live, facebook_id )VALUES (NULL , '".$fbname[0]."', '".md5($fbpasscode)."', '".$fbemail."', '".$fbsession."', '".$ip."', '".DATE_TIME."', 'yes', '".$fbstatus."', '".DATE_TIME."', '3','0','0','1','".DATE_TIME."', 'no', 'OK','off','".$reg_long."','".$reg_lat."','".$reg_country."','".$reg_code."', '0','".eMeetingInput($MSGSTATUS)."','0','no','".$id."')");
	...
	$_POST['skip_pass']=true;
		...
		if(isset($_POST['do'])){ 
			...
			if($Error_Report =="waiting"){
					...
					header("location: ".DB_DOMAIN."index.php?dll=register&sub=activation");
			...
			//echo "SELECT * FROM `members` WHERE `facebook_id`='".$user->id."' LIMIT 1";
...
//if(FACEBOOK_PHOTO == "yes") {
	...
	if($copy){
		...
		//echo "INSERT INTO `files` ( `id` , aid, `user` , `uid` , `date` , `title` , `description` , `bigimage` , `width` , `height` , `filesize` , `views` , `medwidth` , `medheight` ,  `approved` , `rating` , `default` , `featured` , `type` , rating_votes, adult_content) VALUES (NULL , '".$aid."', '".$fbname[0]."', '$userid', '".DATE_NOW."', '".$fbname['0']."', '".$fbname['0']."' , '$profile_image_name', '".$sizes[0]."' , '".$sizes[1]."' , '".filesize($profile_image_path)."', '0', '0', '0', '".$appValue."', '0.00', '1', 'no', 'photo','0','no')";
...
if(FACEBOOK_PHOTO == "yes") {
	...
	*/
	...
	case "friends":{
		...
		header("location: ".DB_DOMAIN."index.php?dll=search&displaytype=detail&friendid=".$_SESSION['uid']);
	...
	case "search":{
		...
		if(!isset($_SESSION['admin_auth']) && isset($PACKAGEACCESS[$_SESSION['packageid']]) && in_array("chatroom-profile",$PACKAGEACCESS[$_SESSION['packageid']]) ){
			...
			header("location: ".DB_DOMAIN."index.php?dll=subscribe"); exit();
		...
		if(isset($_POST['page']) > 2 && $_SESSION['auth'] !="yes"){
			...
			header("location: index.php?dll=login");
	...
	$sub_page = "";
		...
		if(isset($sub_page) && in_array(strip_tags($sub_page),$Access_Page_Account)){
			...
			if($sub_page =="view"){
				...
				header("location: ".DB_DOMAIN."index.php?dll=profile&item_id=".$_SESSION['uid']);
		...
		if(isset($sub_page) && in_array(strip_tags($sub_page),$Access_Page_Messages)){
			...
			if($sub_page =="inbox" || $sub_page =="trash" || $sub_page =="sent" || $sub_page =="wink"){
				...
				if($msgData['1']['id'] != ''){
					...
					header("location: ".DB_DOMAIN."index.php?dll=messages");
			...
			if($sub_page =="manage" && isset($_GET['aid'])){
				...
				*/	
				...
				if(isset($_GET['eid'])){
						...
						if(!isset($FileArray['title'])){ header("location: ".DB_DOMAIN."index.php?dll=gallery&sub=albums");exit();} // STOP HACKING
	...
	*/
	...
	case "games":{
		...
		if(isset($sub_page) && in_array(strip_tags($sub_page),$Access_Page_Affiliate)){
			...
			header("location: ".DB_DOMAIN."index.php?dll=games&sub=search");
	...
	*/
	...
	case "videos":{
		...
		if(isset($sub_page) && in_array(strip_tags($sub_page),$Access_Page_Videos)){
			...
			header("location: ".DB_DOMAIN."index.php?dll=videos&sub=search");
	...
	*/
	...
	case "music":{
		...
		if(isset($sub_page) && in_array(strip_tags($sub_page),$Access_Page_Calendar)){
			...
			header("location: ".DB_DOMAIN."index.php?dll=music&sub=search");
...
//$PAGE_BLANK_FLAG;
	...
	{
		if(D_TEMP == "v18_ADS" || D_TEMP == "v20_blue" || D_TEMP == "v20_darksnow" || D_TEMP == "v20_peach" || D_TEMP == "v20_red" || D_TEMP == "v18_premium")	
		{
