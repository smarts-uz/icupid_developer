...
*/
function GetProfileMeetMeData($id, $selectedGroup="", $ShowTextGroupsOnly=0){
	...
	while( $field = $DB->NextRow($result1) ){
		...
		if(isset($DataArray[$field['fName']]) !="" && $DataArray[$field['fName']] !="0"){
		...
		elseif($field['fType'] == 3){ // LIST BOX VALUE
			...
			if(isset($DataArray[$field['fName']]) == ""){
		...
		if($ShowTextGroupsOnly ==2){
			...
			if($DataArray[$field['fName']] !=""){ 
						...
						<span class="goR">';
						if($_SESSION['uid'] ==$id){
							$ReturnString .= '<a href="'.getThePermalink('account/edit').'" class="pLink"><img src="'.DB_DOMAIN.'images/DEFAULT/_icons/new/pencil.png" align="absmiddle"> '.$GLOBALS['_LANG']['_edit'].' </a>';
						}
...
function GetProfilesMeetMe ($pageGET){
	...
	if(isset($pageGET['Extra']['online']) && $pageGET['Extra']['online']==1 || ( isset($getData['online']) && $getData['online']== 1 ) ){
		...
		$QQ .=" $listFavs $member_limit GROUP BY ".$GroupByThis." ORDER BY ".$OrderByThis."";
...
if(($NETWORK_ID ==1) || ($NETWORK_ID ==3)){
	...
	$QQ .="INNER JOIN members_network ON ( ( (members.id =  members_network.to_uid AND members_network.uid='".$NETWORKD_FRIEND_ID."' )  OR  (  members.id = members_network.uid AND members_network.to_uid='".$NETWORKD_FRIEND_ID."' )  )  AND members_network.type= ( '".$NETWORK_ID."' ) )";
		...
		$QQ .=" $listFavs GROUP BY ".$GroupByThis." ORDER BY members_network.approved DESC, members_network.id DESC";
...

    {
		...
		///////////////////// FILL OUT BLANKS
    	$meetResult = $DB->Row("SELECT count(*) as count FROM members_meetme WHERE uid = '".$_SESSION['uid']."' AND  to_uid = ('".$Data['id']."')");
		
		if($meetResult['count'] == '0'){
	...
	return (isset($memberId)) ? $memberId : 0;
