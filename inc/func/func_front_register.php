...
function ChangeDo1($DoCall, $values = false, $Files = false,$obj=false, $mobile="no"){
	...
	$RunExtra.= ", ".$values['FieldName'.$i] ."='".$BuiltArray."'";												
									...
									while($UploadMax < 13){							
											...
											if(( $value['error'] !=4 ) && isset($Files["uploadFile0".$UploadMax]) && isset($Files["uploadFile0".$UploadMax]['type']) && is_array($Files["uploadFile0".$UploadMax]) && $Files["uploadFile0".$UploadMax]['type'] !="" ){ // error 4 = empty file			
