				...
				switch($SwitchValue){
					...
					case "complete": {	
						...
						while($UploadMax < 13){
							...
							$value['error'] = (isset($value['error'])) ? $value['error'] : '';
							
							if(( $value['error'] !=4 ) && isset($Files["uploadFile0".$UploadMax]) && isset($Files["uploadFile0".$UploadMax]['type']) && is_array($Files["uploadFile0".$UploadMax]) && $Files["uploadFile0".$UploadMax]['type'] !="" ){

									// error 4 = empty file			

									$values['title'] = (isset($values['title'])) ? $values['title'] : '';
									$values['comments'] = (isset($values['comments'])) ? $values['comments'] : '';
						...
						$DB->Insert("INSERT INTO `messages` ( `uid` , `mailnum` , `mail2id` , `mailstatus` , `maildate` , `mailtime` , `mail_subject` , `mail_message` , `mail_displayalert`, my_box, to_box ) VALUES ('0', NULL , '".$_SESSION['uid']."', 'unread', NOW(), NOW(), '".eMeetingInput($Subject)."', '".eMeetingInput($Message)."', '1', 'sent', 'inbox')");
...
function ChangeDo2($DoCall, $values = false, $Files = false,$obj=false, $mobile="no"){
	...
	if(in_array($DoCall, $DoArray)){
		...
		$SwitchValue = ValidateAccountSlide($values,$Files, $obj, $mobile);
...
function ChangeDoValidate($DoCall, $values = false, $Files = false,$obj=false, $mobile="no"){
...
function AddMember($data, $extra){
	...
	*/
	...
	$reg_type = (isset($data['reg_type'])) ? $data['reg_type'] : 'FB';
	...
	if(!isset($_SESSION['clever_ip_country']) && MAPS_ID !="" && GOOGLE_MAPS_KEY !=""){
		...
		if(is_array($exe_data) && $exe_data != 0){			
			...
			if(isset($exe_data[4]) && isset($exe_data[3]) && (is_float($exe_data[4]) || is_numeric($exe_data[4])) && (is_float($exe_data[3]) || is_numeric($exe_data[3]))){
	...
	$DB->Insert("INSERT INTO `members` ( `id` , `username` , `password` , `email` , `session` , `ip` , `lastlogin` , `reg_type`, `visible` , active, `created`, packageid, hits, profile_complete, templateid, updated, moderator, activate_code, highlight, ip_long, ip_lat, ip_country, ip_code,member_rating,  msgStatus,  video_duration,  video_live )

				VALUES (NULL , '".$user."', '".$passcode."', '".$email."', '".$session."', '".$ip."', '".DATE_TIME."', '".$reg_type."', 'yes', '".$status."', '".DATE_TIME."', '".$member_Package_id."','0','100','1','".DATE_TIME."', 'no', 'OK','off','".$reg_long."','".$reg_lat."','".$reg_country."','".$reg_code."', '0','".eMeetingInput($MSGSTATUS)."','0','no')");
	...
	$DB->Update("DELETE FROM members_data_pending_approval WHERE uid= '".$userid."' LIMIT 1");
	$DB->Update("INSERT INTO members_data_pending_approval(uid) VALUES ('".$userid."')");

	// make default values
	$DB->Update("UPDATE `members_data_pending_approval` SET age='1974-JAN-15', country='".eMeetingInput($default_CC)."', headline='' WHERE uid='".$userid."' LIMIT 1"); 

	$DB->Update("UPDATE members_data_pending_approval SET uid= ( '".$userid."' ) ".$extra."  WHERE uid = ( '".$userid."' ) LIMIT 1");
...
$mycountry = $D2['country'];
	...
	$ACTIVATION_CODE = isset($ACTIVATION_CODE) ? $ACTIVATION_CODE : '';
...
/*if ($_POST["recaptcha_response_field"]) {
	...
	for($i = 1; $i < 200; $i++) { 
		...
		if(isset($data['FieldName'.$i]) && $data['FieldName'.$i] == "age"){
		...
		if(isset($data['FieldName'.$i]) && $data['FieldName'.$i] == "headline"){
		...
		if(isset($data['FieldName'.$i]) && $data['FieldName'.$i] == "description"){



			if( isset($data['FieldValue'.$i]) && $data['FieldValue'.$i] == "" ){
...
function ValidateAccountSlide($data, $file, $obj, $mobile="no"){
	...
	*/	
	...
	if ( strlen( $data['email'] ) < 7 || !filter_var($data['email'], FILTER_VALIDATE_EMAIL)){
