...
$approveThis="yes";
					...
					$DB->Insert("INSERT INTO `interests_members` (`group_id` ,`uid` ,`date_joined`)VALUES ('".$new_groupid."', '".$_SESSION['uid']."', '".date("Y-m-d")."')");
			...
			*/
			...
			$values = array_map('eMeetingInput', $values);
				...
				$DB->Update("INSERT INTO `interests_topics` (`uid` ,`interests_id` ,title,`comments` ,`date`) VALUES 
			...
			*/
			...
			case "remove": {
					...
					$DB->Update("DELETE FROM `interests_members` WHERE uid='".$_SESSION['uid']."' AND group_id = ( '".$values['item_id']."' ) ");
					$DB->Update("DELETE FROM `interests_topics` WHERE uid='".$_SESSION['uid']."' AND interests_id = ( '".$values['item_id']."' ) ");
			...
			*/
			...
			case "join": {
				...
				$sql2 = "SELECT * FROM interests_members WHERE uid='".$_SESSION['uid']."' AND group_id = ( '".$values['item_id']."' ) LIMIT 1";
				...
				if ( isset($result2['uid']) && $result2['uid'] == $_SESSION['uid'] ) {			
					...
					$DB->Update("INSERT INTO `interests_members` (`group_id` ,`uid` ,`date_joined`)VALUES ('".$values['item_id']."', '".$_SESSION['uid']."', '".date("Y-m-d")."')");
