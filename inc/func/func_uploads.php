...
function UploadFile($FILE, $logid, $title, $desc, $default_image, $type, $aid, $adult_file){
	...
	if($type == 'music'){
		...
		//$UploadPath = '/home/advandatedev/public_html/advandate.biz/uploads/images/';
		...
		*/
		...
		if(APPROVE_FILES =="yes"){ $appValue = "no"; }else{ $appValue = "yes";	}
			...
			if($aid =="new"){
				...
				$DB->Insert("INSERT INTO `album` ( `aid` , `uid` , `title` , `comment` , `filecount` , `cat` , `allow_f` , `allow_h` , `allow_n` , `allow_a`,password ) VALUES (NULL , '".$_SESSION['uid']."', '".$_SESSION['username']." Photo Album', '', '0', 'public', '1', '1', '1', '1','')");
...
function PhotoValidation($Photo_Upload){
	...
	$exts = array('gif' => IMAGETYPE_GIF, 'jpg' => IMAGETYPE_JPEG, 'png' => IMAGETYPE_PNG, 'jpeg' => IMG_JPEG);
