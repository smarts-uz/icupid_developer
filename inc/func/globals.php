...
function Build_HeaderScripts($page, $sub, $CUSTOM_PLUGIN_HEADER="",$mobile){
	...
	if(D_TEMP =='v17red' || $page == "index"){ //  
		...
		$ReturnData .= "\n\n".'<link rel="stylesheet" href="'.DB_DOMAIN.'inc/css/_version_13.css" type="text/css">'."\n";
                  $ReturnData .= '<link rel="stylesheet" href="'.DB_DOMAIN.'inc/css/jquery.fancybox.min.css" type="text/css">'."\n";	
	}
	else{
...
function DisplayQuickBrowse(){
	...
	$result1 = $DB->Query("SELECT field_groups.id, field_groups.caption FROM field_groups"
                ...
                . "WHERE $extra AND field.quickbrowsepage='yes' GROUP BY"
                . " field_groups.id ORDER BY field_groups.forder ASC");
				...
				if($groups['fName'] =="age"){
					...
					if($groups['fName'] == 'location'){
						...
						$ReturnString .= "</div></div>";
                                        ...
                                        else if($groups['fName'] == 'milesfrom'){

						$ReturnString .= "<div id='Link".$groups['fid']."' class='col-md-12'>
							";
						$ReturnString .= "<input style='width:78px;' name='SeV[".$NumFields."]' type='text' maxlength='20' >";
						$ReturnString .= "&nbsp;&nbsp;Miles From</div>";
					}
					else{
        ...
        case 'search':
            ...
            default:
                if(isset($sub_page) && $sub_page!=""){
                	 if(isset($LANG_HEADINGS[$page.'_'.$sub_page]))
                         {
                	$return .= '<h2 class="template_color">'.$LANG_HEADINGS[$page.'_'.$sub_page].'</h2>';
                         }
                }
                ...
                switch ($page) {
                    ...
                    default:
                        if(isset($sub_page) && $sub_page!=""){
                             ...
                             $return .= '<li><a href="'.DB_DOMAIN.''.$LANG_BREADCRUMB[$page].'">'.$LANG_BREADCRUMB[$page].'</a><span class="breadcrumb-space template_color"></span></li>';
                             if(isset($LANG_BREADCRUMB[$page."_".$sub_page]))
                            {
                               
                        	 $return .='<li>'.$LANG_BREADCRUMB[$page."_".$sub_page].'</li>';
                            }
