...
function DisplaySignupV20IndexFields($id=0){
	...
	*/
...

    {
			...
			if(isset($_POST['FieldValue'.$groups['fid']]) &&  strlen($_POST['FieldValue'.$groups['fid']]) < 2){ $ValStyle="border: 2px solid gainsboro; padding: 2px;";  }else{ $ValStyle="";}
			...
			$ReturnValue .= "<div class='label_left'>".$Caption['caption'].": </div>";
				...
				if($groups['fType'] == 1){
							...
							if(isset($_POST['do']) &&  $_POST['FieldValue'.$groups['fid']] != 1){ $ValStyle="border: 2px solid gainsboro; padding: 2px;"; }else { $ValStyle=""; }
					...
					## age field
						...
						$ReturnValue .= "<div class='col-md-12 home_age'>".MakeAgeListField("",$groups['fid'],$tabcounter)."</div>";
