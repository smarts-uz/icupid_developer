...
?>
		...
		<div id="realtionshiplist" style="float:right; height:30px; line-height:27px; font-size:12px;">
		...
		<a href="<?= getThePermalink('search/friends/'.$_SESSION['uid'].'/5/detail') ?>"><span>My Partner (<?=$MyFriends[4]['total'] ?>)</span></a>
...
<div class="ClearAll"></div>
 ...
 <div id="SearchAlert" class="responce_alert2" style="display:none;"></div>
...
<?=$ThisPersonsNetworkBar ?>
	...
	<form name="SearchResults" method="post" action="<?= getThePermalink('search/view/'.$view_page)?>">
...
<span id="response_search" class="responce_alert2" style="display:  none;"></span>
...
?>
	<div class="result_not_found"><h1><center><?=$GLOBALS['_LANG_ERROR']['_noResults']?></center></h1></div>
	...
	<!-- END DISPLAY -->
		...
		<div id="basic_search">
			<div class="imageframe">
				<div class="highlighted1<? if($Member['featured'] !="yes"){ print "off"; } ?>">
					<a href="<?=$Member['link'] ?>">
						<div class="basic-search-thumb">
							<img src="<?=$Member['image'] ?>" class="thumb" alt="<?=$Member['username'] ?>" width="96" height="96">
						</div>
					</a>
			</div>
			</div>
...
?>
   ...
   <? if(D_STARSIGN ==1){ ?>(<?=$Member['starsign'] ?>)<? } ?> / <? if($Member['genderID'] == 2710){?> <img src="<?=DB_DOMAIN ?>images/DEFAULT/_icons/couple.gif" align="absmiddle"><? } ?><?=$Member['gender'] ?> / <?=$Member['country'] ?> 
	...
	<div class="Acc_ListBox_margin5"> <? if($Member['status'] !=""){ ?>- <?=$Member['username'] ?> <?=$GLOBALS['_LANG']['_pSmsg'] ?> <?=$Member['status'] ?> - <?=ShowTimeSince($Member['lastlogin']); ?> <? } ?> </div></div><div class="Acc_ListBox_right2">
	...
	<? if(!isset($NETWORKD_FRIEND_ID)){ if($_SESSION['uid'] !=$Member['id']){  ?>
        ...
        <? if($Member['onlinenow'] && $_SESSION['uid'] !=$Member['id'] && D_IM ==1 && $_SESSION['auth'] =="yes" ){ ?>
        	<li><img src="<?=DB_DOMAIN ?>images/DEFAULT/_acc/comments.png" width="16" height="16" align="absmiddle">
        	<a <? if(isset($PACKAGEACCESS[$_SESSION['packageid']]) && !in_array("chatroom-im",$PACKAGEACCESS[$_SESSION['packageid']])){ ?> href="javascript:void(0);" onclick="openIMWin(<?=$Member['id'] ?>, '<?=$_SESSION['uid'] ?>','<?=DB_DOMAIN ?>','<?=$IMRoomArray['path'] ?>','<?=$IMRoomArray['width'] ?>','<?=$IMRoomArray['height'] ?>'); return false;" <? }else{ ?> href="<?=getThePermalink('subscribe')?>" <? } ?>>  <? if(D_TEMP != "v17red"){?><? }?><?=$GLOBALS['_LANG']['_pChat'] ?>  <? if(D_TEMP != "v17red")	{?> <? }?></a></li> <? } ?>
        
            <li><img src="<?=DB_DOMAIN ?>images/DEFAULT/_acc/email.png" width="16" height="16" align="absmiddle"> <a <? if($_SESSION['auth'] =="yes"){ ?> <? if(is_array($PACKAGEACCESS[$_SESSION['packageid']]) && !in_array("messages-create",$PACKAGEACCESS[$_SESSION['packageid']])){ ?> href="<?= getThePermalink('messages/create/'.$Member['username']) ?>" <? }else{ ?> href="<?= getThePermalink('subscribe') ?>" <? } ?> <? }else{ ?> href="<?= getThePermalink('login') ?>"<? } ?>><?=$GLOBALS['LANG_COMMON'][9] ?></a></li>
			<? if(D_HOTLIST ==1){ ?><li><img src="<?=DB_DOMAIN ?>images/DEFAULT/_acc/heart.png" width="16" height="16" align="absmiddle"> <a <? if($_SESSION['auth'] =="yes"){ ?>href="#"  onclick="ProfileAddNet(<?=$Member['id'] ?>,1); alert('<?=$GLOBALS['_LANG']['_updated'] ?>'); return false;" <? }else{ ?>href="<?= getThePermalink('login') ?>"<? } ?>> <?=$GLOBALS['_LANG']['_hotList'] ?></a></li><? } ?>
			<? if(D_FRIENDS ==1){ ?><li><img src="<?=DB_DOMAIN ?>images/DEFAULT/_icons/new/user_green.png" width="16" height="16" align="absmiddle"> <a <? if($_SESSION['auth'] =="yes"){ ?>href="#"  onclick="ProfileAddNet(<?=$Member['id'] ?>,2);alert('<?=$GLOBALS['_LANG']['_updated'] ?>');return false;" <? }else{ ?>href="<?= getThePermalink('login') ?>"<? } ?>><?=$GLOBALS['_LANG']['_friendsList'] ?></a></li><? } ?>
			<? if(D_WINK ==1){ ?><li><img src="<?=DB_DOMAIN ?>images/DEFAULT/_acc/emoticon_smile.png" width="16" height="16" align="absmiddle"> <a <? if($_SESSION['auth'] =="yes"){ ?> <? if(is_array($PACKAGEACCESS[$_SESSION['packageid']]) && !in_array("chatroom-wink",$PACKAGEACCESS[$_SESSION['packageid']])){ ?> href="#" onclick="openQuickWink(<?=$Member['id'] ?>,'<?=$Member['username'] ?>','<?=$Member['image'] ?>'); return false;" <? }else{ ?> href="<?= getThePermalink('subscribe') ?>" <? } ?> <? }else{ ?> href="<?= getThePermalink('login') ?>"<? } ?>><?=$GLOBALS['LANG_COMMON'][10] ?></a></li><? } ?>
			<? if(D_FOLLOW ==1){ ?><li><img src="<?=DB_DOMAIN ?>images/DEFAULT/_acc/add.png" align="absmiddle"> <a <? if($_SESSION['auth'] =="yes"){ ?>href="#"  onclick="ProfileAddNet(<?=$Member['id'] ?>,8); alert('<?=$GLOBALS['_LANG']['_updated'] ?>'); return false;" <? }else{ ?>href="<?= getThePermalink('login') ?>"<? } ?>>Follow Me</a></li> <? } ?>
	 	</ul></div>
...
function MakeSearchOptions(newtoday, birthday, fav, onlinenow, highlight, featured, pics){
 ...
 <script>
  ...
  google.maps.event.addDomListener(window, 'load', function() {  
    initialize('quickSearchLocationNew');
    });
