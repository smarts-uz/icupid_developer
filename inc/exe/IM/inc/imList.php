...
// get chat messages
$RunImQuery = $DB->Query("SELECT members.video_live, members.msgStatus, members_data.location, members_data.age, members_data.gender, members_online.page, members.id,  members.username, files.bigimage, files.approved, files.title, files.description, files.adult_content FROM members INNER JOIN members_online ON ( members_online.logid = members.id AND members_online.logid !=0 AND members_online.logid != ('".$_SESSION['uid']."' ) ) INNER JOIN members_data ON ( members_data.uid = members.id ) INNER JOIN members_privacy ON (members.id = members_privacy.uid AND members_privacy.IM ='yes' ) LEFT JOIN files ON ( files.uid = members.id AND files.default=1 AND files.approved='yes') WHERE members.visible = 'yes' AND members.active ='active' $QueryExtra GROUP BY members.id ORDER BY members.lastlogin DESC LIMIT 30");
...
<div class="imspan <?php if($i % 2){ ?>search_display_off<? }else{ ?>search_display_on<? } ?>" id="div_<?=$value['id'] ?>" style="margin-top:10px; overflow:hidden; width:200px; ">
	...
	<div class="info" style="margin-top:10px;font-size:11px;"> 
		<?php if(isset($_SESSION['IM_GEN_ARRAY'][$IMData['gender']]['caption'])) { ?>
				<?=$_SESSION['IM_GEN_ARRAY'][$IMData['gender']]['caption'] ?> /
		<?php } ?>
