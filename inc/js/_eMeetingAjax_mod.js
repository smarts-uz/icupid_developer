 ...
 *	 VERSION: 9
 *	 LISENSE: OWN / LEASED (https://www.advandate.com)
 *
	...
	{				
		...
		Ajax.mRequest.onreadystatechange = function() {
			if(Ajax.mRequest.readyState == 4 && Ajax.mRequest.status == 200){
				//alert(Ajax.mRequest.responseText);
...
function reportMember(uid,touid){

	var confrm = confirm("Are you sure you want to report this profile?");

	if(confrm){

		eMeetingDo('inc/ajax/_actions.php?action=ReportMember&uid='+uid+'&touid='+touid,'reportMember');
		document.getElementById("reportMember").style.display = 'block';

	}
}

function ChangeRelationship(div, id1, id2, spandiv){
	//alert("newId="+div+" old="+id1+" uid="+id2+" spandiv="+spandiv+"")
	//alert(div, id1, id2, spandiv);
...
function ChangeRelationshipDiv(new1, old1, uid, spandiv){ 
	...
	setTimeout(ChangeRelationshipResult(old1), 11000);
}

function ChangeRelationshipResult(old_1){ 

	
	
	eMeetingDo('/inc/ajax/_actions.php?action=UpdateRelationshipCount&newolidd='+old_1,'Results');
	eMeetingDo('/inc/ajax/_actions.php?action=ChangeRelationshipList','realtionshiplist');
...
function DeleteNetwork(userid, netid){
	...
	document.getElementById('response_search').style.display='block'; 
...
function meetMeYes(userid, touserid){
	Timer_Icon('response_message');
	eMeetingDo('inc/ajax/_actions.php?action=MeetMeYes&uid='+userid+'&touserid='+touserid,"response_message");
	document.getElementById("MeetMeSearch").submit();
}

function meetMeNo(userid, touserid){
	Timer_Icon('response_message');
	eMeetingDo('inc/ajax/_actions.php?action=MeetMeNo&uid='+userid+'&touserid='+touserid,"response_message");
	document.getElementById("MeetMeSearch").submit();
}
...
function Timer_Icon_parent(divlayer){
	var mydomain = window.location.hostname;
	window.opener.document.getElementById(divlayer).innerHTML='<img src="//'+mydomain+'/images/DEFAULT/load_red.gif" width="16" height="16">';
}
...
function ProfileSendWink(id){
	...
	document.getElementById('profile_responce_span').style.display='block';
...
function DeleteBlogPost(id){
	Timer_Icon('response_search');
	eMeetingDo('/inc/ajax/_actions.php?action=DeleteBlogPost&blogid='+id,"response_search");
}
...
function eMeetingLinkedField(value, linkedID, searchPage){
	...
	eMeetingDo('//'+mydomain+'/inc/ajax/_actions.php?action=PopLinkedField&lid='+linkedID+'&value='+value+'&rownum='+searchPage,'Link'+linkedID);
	//alert(eMeetingDo('//'+mydomain+'/inc/ajax/_actions.php?action=PopLinkedField&lid='+linkedID+'&value='+value+'&rownum='+searchPage,'Link'+linkedID));
}
...
window.setInterval(function(){
	eMeetingDo('/inc/ajax/_actions.php?action=DisplayPopupNotification',"popup-notificaiton-section");

	setTimeout(function(){
		eMeetingDo('/inc/ajax/_actions.php?action=UpdatePopupNotification',"response_search");		
	},5000);

}, 10000);

function UpdateVideoChat(chat_id){

	eMeetingDo('/inc/ajax/_actions.php?action=UpdatePopupNotification&chat_id='+chat_id,"response_search");		
...
window.setTimeout(function(){
	...
	var xhttp = new XMLHttpRequest();
  	...
  	xhttp.open("GET", "/inc/ajax/_actions.php?action=VisitorEntry", true);
  	xhttp.send();
},10000);
...
//function nextvalidation(id){
	...
	//lert(id);
	//eMeetingDo('inc/ajax/_actions.php?action=nextvalidation&id='+id,"response_searchnext");
