...
if (D_FRIENDS == 1) {
   ...
   $friendlist = "";
	...
	case "ReportMember":{
		...
		$uid 		= trim(strip_tags($_GET['uid'])); 		// friend type
		$to_uid 		= trim(strip_tags($_GET['touid'])); 		// user id

		$reported  = $DB->Row("SELECT count(*) as count FROM members_reported WHERE uid = '".$uid."' AND to_uid = '".$to_uid."'");
		
		if($reported['count'] > 0){
			print "This profile is already reported by you.";	
		}
		else{
			$DB->Update("INSERT into members_reported (uid, to_uid, `date`, visible) values ('".$uid."', '".$to_uid."', '".date("Y-m-d")."', 'no')");
			print "This profile reported successfully.";
		}
	} break;
	...
	case "UpdateRelationship":{
		...
		$oid 	= trim(strip_tags(isset($_GET['olid']))); 	// user id
		//print_r($oid);
		$nid 	= trim(strip_tags($_GET['nid'])); 	// user id 
		//print_r($nid);
		$muid 	= trim(strip_tags($_GET['muid'])); 	// user id 
		//print_r($muid);
		//echo "UPDATE members_network SET members_network.type='".$nid."' WHERE uid='".$_SESSION['uid']."' AND to_uid='".$muid."' LIMIT 1";
	...
	//Totalcount UpdateRelationshipCount

	case "UpdateRelationshipCount":{

		$totalid 	= $_GET['newolidd']; 

		$Totalvalu = $DB->Row("SELECT count(`uid`) FROM members_network WHERE members_network.type='".$totalid."' AND uid='".$_SESSION['uid']."'");

		Print '<span class="a1" style="font-size:14px;line-height:35px;"> <b>'.$Totalvalu[0].'</b> Results </span>';
	
	} break;

	//realtionshiplist ChangeRelationshipList

	case "ChangeRelationshipList":{

		$getFriends = $DB->Row("SELECT count(`uid`) FROM members_network WHERE members_network.type=2 AND uid='".$_SESSION['uid']."'");

		$getHotList = $DB->Row("SELECT count(`uid`) FROM members_network WHERE members_network.type=1 AND uid='".$_SESSION['uid']."'");

		$getBlockList = $DB->Row("SELECT count(`uid`) FROM members_network WHERE members_network.type=3 AND uid='".$_SESSION['uid']."'");
	
		
		Print '<a href="https://www.advandate.biz/search/friends/'.$_SESSION['uid'].'/2/detail"><span>My Friends List ('.$getFriends[0].')</span></a>   -		 <a href="https://www.advandate.biz/search/friends/'.$_SESSION['uid'].'/1/detail"><span>My Hot List ('.$getHotList[0].')</span></a> -		 <a href="https://www.advandate.biz/search/friends/'.$_SESSION['uid'].'/3/detail"><span>Blocked List ('.$getBlockList[0].')</span></a> ';
	...
	if(is_numeric($_GET['uid']) && is_numeric($_GET['touserid'])){
	
		$DB->Insert("INSERT INTO members_meetme(uid,to_uid,date,approved) values('".$_GET['uid']."','".$_GET['touserid']."','".date("Y-m-d")."','yes')");
		
		$usersendto = $DB->Row("SELECT username,email FROM members WHERE id = '".$_GET['touserid']."'");

		$userfrom = $DB->Row("SELECT username,email FROM members WHERE id = '".$_GET['uid']."'");

		$Data = array();

		$Data['email'] =  $usersendto['email'];
		$Data['username'] =  $usersendto['username'];																	
		$Data['meet_me_username'] =  $userfrom['username'];																	
		$Data['meet_me_profile_link'] =  DB_DOMAIN.$userfrom['username'];																	

		SendTemplateMail($Data, 64);

		print $GLOBALS['_LANG_ERROR']['_meetmeyes'];
	...
	if(is_numeric($_GET['uid']) && is_numeric($_GET['touserid'])){
		...
		print $GLOBALS['_LANG_ERROR']['_meetmeno'];
	...
	//case 'nextvalidation': {
		...
		//$id= trim(strip_tags($_GET['id']));		
  		...
  		//$nextdata = $DB->Query("SELECT field.fid,field.fType, field.fName,field.linked_id FROM field INNER JOIN field_groups ON ( ( field_groups.id = field.groupid  || field_groups.id = field.groupid_1 || field_groups.id = field.groupid_2 )  ) WHERE field.fName NOT IN ('country','em_85820081128') AND ( field.groupid = '".$id."' OR field.groupid_1 = '".$id."' OR field.groupid_2 = '".$id."') GROUP BY field.fid ORDER BY field.fOrder ASC");
  		//while ($field = $DB->NextRow($nextdata)) {

  			//print_r($field);
  			//if($field['FieldValue29'] == 1){	
	...
	*/
				...
				case "displayPackageFea":{
					...
					foreach($PAGE_ARRAY as  $PAGENAME => $TOP_MENU){
						...
						print '<li><a href="javascript:void();" onclick="return false;">Extra '.$result['SMS_credits'].' '.$lang_ajax[43].'</a></li>';
					}						
					print '</ul>';
...
*/
	...
	## switch comments page
		...
		//if($val['Notifications'] =="yes"){
							...
							$Data['custom'] 			=  "<a href='".DB_DOMAIN."/account/comments'>".DB_DOMAIN."</a>";													
		...
		if(D_TEMP=='v17red')
		{
			...
			<div class="pic50"><a class="photo_50" href="'.getThePermalink('user/'.$_SESSION['username']).'" style="float:left;"><img src="'.DB_DOMAIN."inc/tb.php?src=".$img.'&x=48&y=48" alt="'.$_SESSION['username'].'"  class="img_border" width="48" height="48"/></a></div>
			</div>
		...
		{
				print '<div class="topic_reply"><div class="topic_reply_av">
			<div class="pic50"><a class="photo_50" href="'.getThePermalink('user/'.$_SESSION['username']).'"><img src="'.DB_DOMAIN."inc/tb.php?src=".$img.'&x=48&y=48" alt="'.$_SESSION['username'].'"  class="img_border" width="48" height="48"/></a></div>
			</div>
...
case "CheckAlbumPassword": {
	...
	if(CheckAlbumAccess($aid, $password)){
		...
		print "Thank you. <a href='".getThePermalink('profile/manage',array('item_id' => $pid,'item2_id' => $aid))."' style='color:red;font-weight:bold;font-size:15px'>Please click here to view the album </a>";
...
//////////////////// LINKED LIST BOXES ///////////////////////
	...
	case "ProfileNetwork":{
		...
		if($_SESSION['auth'] != "yes"){
			...
			if($result['found']  > 0){
				...
				$val = $DB->Row("SELECT members.username, members.email, members_privacy.friends FROM members_privacy INNER JOIN members ON (members_privacy.uid = members.id ) WHERE members_privacy.uid=('".$id."') LIMIT 1");
				...
				if($netid ==2){ // dont send if its a block list value
					...
					$val['custom']  = "<a href='".DB_DOMAIN."friends'>".DB_DOMAIN."friends</a>"; // Must be above the admin_email
	...
	*/
	...
	case "PopLinkedField" : {
		...
		$sliding_class = '';
		$style = 'width:185px; overflow: hidden;'; 
		if(D_USER_REGISTRATION == 'sliding'){
			$sliding_class = ' form-control col-md-12';
			$style = ''; 
		}

		$ReturnString .= "<select class='sc_select_a $sliding_class' name='".$FieldNameString."'  style='$style' ".$LinkedCode.">";							
	...
	case "DisplayNetworkStates" : {
		
		$country_id = $_GET['country_id'];
		$field_id = $_GET['fid'];
		if(D_USER_REGISTRATION == 'sliding'){
			$cls = "class='col-md-12 form-control'";
		}
		else{
			$cls = "";
		}
		echo "<select name='FieldValue".$field_id."' onChange='eMeetingNetworkCities(this.value, 26);' $cls>";

		echo "<option value='0'>------------------</option>";
		echo DisplayNetworkStates($country_id,0);
		echo "</select>";
		return;
		
	} break;

	case "DisplayNetworkCities" : {
		
		$state_id = $_GET['state_id'];
		$field_id = $_GET['fid'];
		
		if(D_USER_REGISTRATION == 'sliding'){
			$class = "class='col-md-12 form-control'";
		}
		else{
			$class = "";
		}
		echo "<select name='FieldValue".$field_id."' $class>";

		echo "<option value='0'>------------------</option>";
		echo DisplayNetworkCities($state_id,0);
		echo "</select>";
		return;
		
	} break;
	
	case "DisplayPopupNotification" : {
		
		require_once "../func/func_profile_page.php";
		//require_once "../API/api_functions.php";

		$SQL_MESSAGES = "SELECT messages.uid, messages.mailnum, members.username,files.bigimage, files.type FROM messages LEFT JOIN members ON ( members.id = messages.uid ) LEFT JOIN files ON ( files.uid = messages.uid AND files.approved = 'yes' AND files.default=1 AND files.type = 'photo' )
		WHERE mail2id='".$_SESSION['uid']."' AND mailstatus='unread' AND to_box='inbox' AND messages.type != 'wink' AND messages.popup_viewed = 'no'";

		$SQL_REQUEST = "SELECT members.id,members.username,files.bigimage,files.type FROM members_network LEFT JOIN members ON ( members.id = members_network.uid AND members.active = 'active' ) LEFT JOIN files ON ( files.uid = members.id AND files.approved = 'yes' AND files.default=1 AND files.type = 'photo' ) WHERE members.visible = 'yes' AND members_network.to_uid='".$_SESSION['uid']."' AND (members_network.type=2 OR members_network.type=4) AND members_network.approved !='yes' AND members_network.popup_viewed ='no'";

		$SQL_COMMENTS = "SELECT members.id, members.username,files.bigimage,files.type FROM comments LEFT JOIN members ON comments.from_uid = members.id LEFT JOIN files ON ( files.uid = members.id AND files.approved = 'yes' AND files.default=1 AND files.type = 'photo' ) WHERE comments.to_uid='".$_SESSION['uid']."' AND comments.approved != 'yes' AND comments.popup_viewed='no'";


	 

		$SQL_WINKS = "SELECT messages.uid, messages.mailnum, members.username,files.bigimage, files.type FROM messages LEFT JOIN members ON ( members.id = messages.uid ) LEFT JOIN files ON ( files.uid = messages.uid AND files.approved = 'yes' AND files.default=1 AND files.type = 'photo' ) WHERE mail2id='".$_SESSION['uid']."' AND mailstatus='unread' AND to_box='inbox' AND messages.type = 'wink' AND messages.popup_viewed = 'no'";
		
		
		$rltMessages 	= $DB->Query($SQL_MESSAGES);
		$rltRequest 	= $DB->Query($SQL_REQUEST);
		$rltComments 	= $DB->Query($SQL_COMMENTS);
		$rltWinks 		= $DB->Query($SQL_WINKS);
		

		/* MESSAGES */
		while( $DataArray = $DB->NextRow($rltMessages) ){

		?>

			<div class="popup-notificaiton">
		        <div class="popup-image">
		            <img src="<?=ReturnDeImage($DataArray,"medium");?>">
		        </div>
		        <div class="popup-content">
		            <p>You just received a message from <?=$DataArray['username']?></p>
		            <img src="<?=DB_DOMAIN?>images/n_messages.png">
		            <a href="<?=DB_DOMAIN?>messages/read/<?=$DataArray['mailnum']?>" class="background_color">Read</a> 
		        </div>
		    </div>

		<?
		
		//$DB->Update("UPDATE messages SET popup_viewed = 'yes' WHERE uid='".$DataArray['uid']."' AND mail2id='".$_SESSION['uid']."' AND mailstatus='unread' AND to_box='inbox' AND messages.type = 'normal' AND popup_viewed = 'no'");
		
		//$DB->Update("UPDATE messages SET popup_viewed = 'yes' WHERE mailnum='".$DataArray['mailnum']."'");

		}


		/* FRIEND REQUESTS */
		echo"<div id='response_search'></div>";	
		while( $DataArray = $DB->NextRow($rltRequest) ){
		?>
			...
			<div class="popup-notificaiton">
		        ...
		        <div class="popup-content">
		            <p>You just received a friend request from <?=$DataArray['username']?></p>
		            <img src="<?=DB_DOMAIN?>images/n_friends.png">
 					<a href="javascript:void(0);" style="padding: 6px 10px;margin-left: 5%; font-size: 12px;" class="background_color" onclick="ApproveNetwork(<?=$DataArray['id']?>,2); return false;">Approve</a>
		            <a href="javascript:void(0);" style="padding: 6px 10px;margin-left: 5%; font-size: 12px;" class="background_color" onclick="DeleteNetwork(<?=$DataArray['id']?>,2); return false;">Remove</a>
		...
		//$DB->Update("UPDATE members_network SET popup_viewed = 'yes' WHERE to_uid='".$_SESSION['uid']."' AND uid='".$DataArray['id']."' AND (type=2 OR type=4) AND popup_viewed ='no'");

		}

		/* COMMENTS */
		while( $DataArray = $DB->NextRow($rltComments) ){
		?>
			...
			<div class="popup-notificaiton">
		        ...
		        <div class="popup-content">
		            <p>You just received a comment from <?=$DataArray['username']?></p>
		            <img src="<?=DB_DOMAIN?>images/n_comments.png">
		            <a href="<?=DB_DOMAIN?>account/comments" class="background_color">View</a> 
		        </div>
		    </div>
		...
		//$DB->Update("UPDATE comments SET popup_viewed = 'yes' WHERE from_uid = '".$DataArray['id']."' AND to_uid='".$_SESSION['uid']."' AND approved != 'yes' AND popup_viewed ='no'");
		}

		/* WINKS */
		while( $DataArray = $DB->NextRow($rltWinks) ){
		?>
			...
			<div class="popup-notificaiton">
		        ...
		        <div class="popup-content">
		            <p>You just received a wink from <?=$DataArray['username']?></p>
		            <img src="<?=DB_DOMAIN?>images/n_winks.png">
		            <a href="<?=DB_DOMAIN?>messages/read/<?=$DataArray['mailnum']?>" class="background_color">Read</a>
		...
		<?
		//$DB->Update("UPDATE messages SET popup_viewed = 'yes' WHERE uid='".$DataArray['uid']."' AND mail2id='".$_SESSION['uid']."' AND mailstatus='unread' AND to_box='inbox' AND messages.type = 'wink' AND popup_viewed = 'no'");

		}
		if(!isset($PACKAGEACCESS[$_SESSION['packageid']]) || !in_array("chat-chat",$PACKAGEACCESS[$_SESSION['packageid']]) ) {

			$SQL_VCHAT = "SELECT members_videochat.id AS chat_id,members_videochat.uid AS uid, members.username,files.bigimage, files.type FROM members_videochat LEFT JOIN members ON ( members.id = members_videochat.uid ) LEFT JOIN files ON ( files.uid = members_videochat.uid AND files.approved = 'yes' AND files.default=1 AND files.type = 'photo' ) WHERE members_videochat.to_uid='".$_SESSION['uid']."' AND members_videochat.read='no'";
			$rltVchat 		= $DB->Query($SQL_VCHAT);
			while( $DataArray = $DB->NextRow($rltVchat) ){
			?>
				...
				<div class="popup-notificaiton">
			        ...
			        <div class="popup-content">
			            <p><?=$DataArray['username']?> sent you video chat request.</p>
			            <img src="<?=DB_DOMAIN?>images/n_winks.png">
			            <a href="javascript:void();" onclick="ShowHideVideoSection('block','<?=DB_DOMAIN?>chat/web?receive_id=<?=$DataArray['chat_id']?>');" class="background_color">Start</a>

			            <a href="javascript:void();" onclick="UpdateVideoChat(<?=$DataArray['chat_id']?>);" class="background_color">Reject</a>
	...
	case "UpdatePopupNotification" : {
		...
		$SQL_COMMENTS = "SELECT members.id as id, comments.id as cid, members.username,files.bigimage,files.type FROM comments LEFT JOIN members ON comments.from_uid = members.id LEFT JOIN files ON ( files.uid = members.id AND files.approved = 'yes' AND files.default=1 AND files.type = 'photo' ) WHERE comments.to_uid='".$_SESSION['uid']."' AND comments.approved != 'yes' AND comments.popup_viewed='no'";
		...
		while( $DataArray = $DB->NextRow($rltComments) ){
			...
			$DB->Update("UPDATE comments SET popup_viewed = 'yes' WHERE id = '".$DataArray['cid']."' AND approved != 'yes' AND popup_viewed ='no'");
		}
		...
		if(isset($_GET['chat_id'])){
			$DB->Update("UPDATE members_videochat SET `read` = 'yes' WHERE id='".$_GET['chat_id']."'");
		}
		
	} break;

	case 'overviewNotifications': {
		$offset = $_GET['offset'];
		
		require_once "../func/func_overview_page.php";

		$notifications = GetOverviewNotifications($offset);
	...
	case "VisitorEntry" : {
		...
		*/
		unset($_SESSION['my_visitor_added']);
		if(!isset($_SESSION['my_visitor_added'])){
			...
			if($DB->Affected() ==0){
				...
				if (!isset($_COOKIE['my_visitor_added']) || (isset($_COOKIE['my_visitor_added']) && $_COOKIE['my_visitor_added'] != date("Y-m-d"))) {
					...
					VALUES ('".$ip."' , '".$_SESSION['visitor_browser']."' , '".date("h")."', '".date("i")."',CURRENT_TIMESTAMP , '".date("d")."', '".date("m")."', '".date("y")."', '".strip_tags(str_replace("'","",isset($_SERVER['HTTP_REFERER'])))."' , '".$_SESSION['visitor_url']."')");
	...
	case "googlesign" : {
		...
		$fbid = $DB->Row("SELECT * FROM members WHERE `email` = '$email' OR (facebook_id='".$id."' and facebook_id != '0' and facebook_id != '') LIMIT 1");
		...
		if(empty($fbid)){
			...
			if($id){	
				...
				$fbstatus = "active";
				$fbpasscode = $id;
				require_once('../func/func_facebook_username.php');
				...
				$DB->Insert("INSERT INTO `members` ( `id` , `username` , `password` , `email` , `session` , `ip` , `lastlogin` , `reg_type`,`visible` , active, `created`, packageid, hits, profile_complete, templateid, updated, moderator, activate_code, highlight, ip_long, ip_lat, ip_country, ip_code,member_rating,  msgStatus,  video_duration,  video_live, facebook_id )VALUES (NULL , '".$fbname['0']."', '".md5($fbpasscode)."', '".$fbemail."', '".$fbsession."', '".$ip."', '".DATE_TIME."', 'Google','yes', '".$fbstatus."', '".DATE_TIME."', '3','0','0','1','".DATE_TIME."', 'no', 'OK','off','".$reg_long."','".$reg_lat."','".$reg_country."','".$reg_code."', '0','".eMeetingInput($MSGSTATUS)."','0','no','".$id."')");
		...
		//	header('Location: '.$new_url);
			...
			$data = $DB->Row("SELECT * FROM members WHERE `email` = '$email' OR (facebook_id='".$id."' and facebook_id != '0' and facebook_id != '') LIMIT 1");
